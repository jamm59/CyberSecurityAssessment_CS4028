...
# VARIABLES
foundCount = len(hashedPasswords) 
alreadySeenPasswordCombination = set({})
...
def cachePasswordHashTable():
    with open("hashTable.pkl", "wb") as f:
        pickle.dump(passwordDictionary, f)

def loadPasswordDictTable():
...
for password in passwordList:
    ...
    hashedTable[hashedPassword] = password

results = []
for hashed in hashedPasswords:
    if hashed in hashedTable:
        results.append(hashedTable[hashed])

print(results)

# pprint.pprint(hashedTable)