...
class TaskThree:
    def __init__(self,inputHashedAndSaltedPair, websocket=None):
        self.inputHashedAndSaltedPair = inputHashedAndSaltedPair
        self.hashedTable = dict({})
        self.passwordList = self.loadPasswordList()
        self.websocket = websocket

    def loadPasswordList(self):
        f = open("PasswordDictionary.txt", "r")
    ...
    async def checkHash(self):
        ...
        for saltedHashedPassword, saltKey in self.inputHashedAndSaltedPair:
            for password in self.passwordList:
                stringToBeHashedLeft = saltKey + password
                stringToBeHashedRight = password + saltKey

                hashedPasswordLeft = hashlib.sha512(stringToBeHashedLeft.encode("utf-8")).hexdigest()
                hashedPasswordRight = hashlib.sha512(stringToBeHashedRight.encode("utf-8")).hexdigest()

                self.hashedTable[hashedPasswordLeft] = password
                self.hashedTable[hashedPasswordRight] = password

        results = []
        for saltedHashedPassword, saltKey in self.inputHashedAndSaltedPair:
            if saltedHashedPassword in self.hashedTable:
...
if __name__ == "__main__":
    task3: TaskThree = TaskThree(inputHashedAndSaltedPair=inputHashedAndSaltedPair)
    task3.checkHash()