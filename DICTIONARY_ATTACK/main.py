...
class TaskTwo:
    def __init__(self, hashedPasswords, websocket=None) -> None:
        # VARIABLES
        self.hashedPasswords = hashedPasswords
        self.foundCount = len(hashedPasswords) 
        self.alreadySeenPasswordCombination = set({})
        self.unhashedPasswords = set({})
        self.passwordList = self.loadPasswordList()
        self.hashedTable = None
        self.websocket = None
    ...
    def cachePasswordHashTable(self):
        with open("hashTable.pkl", "wb") as f:
            self.hashedTable is not None and pickle.dump(self.hashedTable, f)

    def loadPasswordDictTable(self):
        try:
    ...
    def checkHash(self):
        ...
        if self.hashedTable is None:
            ...
            for password in self.passwordList:
                ...
                self.hashedTable[hashedPassword] = password
            self.cachePasswordHashTable()

        results = []
        for hashed in self.hashedPasswords:
            if hashed in self.hashedTable:
...
if __name__ == "__main__":
    task2: TaskTwo = TaskTwo(hashedPasswords=hashedPasswords)
    task2.checkHash()